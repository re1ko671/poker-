...
// Вэб хуудасны файлууд (HTML, CSS, JS) байрлах хавтас
app.use(express.static(__dirname + '/public'));

// Үндсэн хаяг дээр ирэх хүсэлтийг хүлээж авах (Алдаанаас сэргийлнэ)
app.get('/', (req, res) => {
...
const cv = c => { return { r: c % 13 + 2, s: c / 13 | 0 } };
const online = t => [...io.sockets.sockets.values()].some(s => s.data.t == t);

// ---- hand evaluation ----
function ev5(c) {
    const rs = c.map(x => x.r).sort((a, b) => b - a), fl = c.every(x => x.s == c[0].s); let st = 0;
...
function best(c) {
    let b = null; const rec = (i, a) => {
        if (a.length == 5) { const v = ev5(a); if (!b || cmp(v, b) > 0) b = v; return }
        if (i >= c.length) return; rec(i + 1, [...a, c[i]]); rec(i + 1, a)
    }; rec(0, []); return b
}
...
function act(i, t, amt) {
    ...
    if (t == 'f') { p.fold = true; lg(u.name + ' хаялаа') }
        ...
        const to = Math.min(p.bet + u.chips, Math.max(Math.floor(+amt) || 0, H.cur + BB)); if (to <= H.cur) return act(i, 'c');
...
function view(s) {
    ...
    return {
        me, dealer, cur: H ? H.cur : 0, board: H ? H.board : [], rem: H && H.turn >= 0 ? Math.max(0, H.dl - Date.now()) : 0, turn: H && !H.res ? H.turn : -1,
        pot: H ? H.p.reduce((a, p) => a + (p ? p.total : 0), 0) : 0, chips: U[t].chips + (U[t].pend || 0), log: log.slice(0, 6),
        seats: seats.map((tk, i) => {
            if (!tk) return null; const u = U[tk], p = H && H.p[i];
...
io.on('connection', s => {
    ...
    s.on('stand', () => {
        const t = s.data.t, i = seats.indexOf(t); if (i < 0) return;
    ...
    s.on('disconnect', () => { const t = s.data.t; if (!t) return; const i = seats.indexOf(t); if (i >= 0 && !(H && H.p[i])) { seats[i] = null } bc() })
});

// Render дээр автоматаар портыг зөв унших тохиргоо (Зассан хэсэг)
const PORT = process.env.PORT || 3000;
srv.listen(PORT, '0.0.0.0', () => {
    console.log(`Poker server ready on port ${PORT}`);
});
